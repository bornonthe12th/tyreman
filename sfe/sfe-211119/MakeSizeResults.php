...
//Show hub stock for BABush
		...
		while ($i < $num)
			...
			if (mysql_result($srchresult,$i,"decibels") != "") {
				echo "db";
				}
		  	echo "</td>";
					// Show tyre Options columns, currently BAB and Tyreman demo/dev regions only
...
/*Only show label if all label variables have data in them*/
 if (mysql_result($srchresult,$i,"decibels") != "" and 
     mysql_result($srchresult,$i,"fuel_efficiency") != "" and 
     mysql_result($srchresult,$i,"wet_braking") != ""  and 
     mysql_result($srchresult,$i,"vehicle_class") != ""){
