...
if (substr("$dirArray[$index]", 0, 1) != "."){ // don't list hidden files
	...
	while ((($data = fgetcsv($handle, 1000, ",")) !== FALSE) and (strlen($data[0])>2))
		...
		//echo 'Branch = ' ; var_dump($branch); echo "\n";
		...
		//echo 'del_add = ' ; var_dump($del_add); //echo "\n";
    	
    	//if we have a cust update it else insert it
    	if ($cust){ // update
    		//echo "updating cust-" . $cust ."\n\n";
			...
			if ($del_add) {//update del address
				...
				//set up query
				$query="insert into addresses (customer_id,address_line1,address_line2,
					address_line3,address_line4,address_line5,postcode,addressee) values (";	
				$query = $query . "'" . $cust . "',"; 
				$query = $query . "'" . $data[12] . "',"; 
				$query = $query . "'" . $data[13] . "',"; 
				$query = $query . "'" . $data[14] . "',"; 
		...
		$updstmnt="update customers c ";	
		    ...
		    $setstmnt = $setstmnt ."c.show_cust_specials='" . $data[26] . "', ";
			...
			if ($cust) { 
				...
				//insert customer
				$query="insert into customers (title,first_name,surname,phone,mobile,fax,account_no,delivery_add_id,default_branch_id,credit_limit,trade_limit,account_balance,on_stop_flag,enquiry_only,hide_rrp,show_cust_specials) values ";	
			    $query = $query."('" .$data[5] . "',";
			    $query = $query."'". $data[6] . "',";
			    $query = $query."'". $data[7] . "',";
			    $query = $query."'". $data[8] . "',";
